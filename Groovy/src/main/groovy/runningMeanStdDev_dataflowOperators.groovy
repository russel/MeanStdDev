...
operator(inputs: [countInputs], outputs: [count], stateObject: [total: 0i]) {Double x ->
  ++stateObject.total
  bindOutput 0, stateObject.total
}
operator(inputs: [sumInputs], outputs:[sum], stateObject:[total: 0.0d]) {Double x ->
  stateObject.total += x
  bindOutput 0, stateObject.total
}
operator(inputs: [sumSqInputs], outputs: [sumSq], stateObject: [total: 0.0d]) {Double x ->
